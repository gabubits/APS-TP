...
# ------------ MODELAGEM COMEÇA AQUI -----------------------
        ...
        usuarios_label = QLabel("Todos os Usuários")
        usuarios_label.setFont(self.fonte_rotulo)
        usuarios_label.setStyleSheet("color: white; text-center: margin-top:10px; font-size: 20px;")
        layout.addWidget(usuarios_label ,  0, 0, 1, 2, Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignCenter)

        tabela_usuarios = QTableWidget(5, 2)  
        tabela_usuarios.setHorizontalHeaderLabels(["Usuário", "Ação"])
        ...
        for i, (usuario,) in enumerate(dados_usuarios):
            tabela_usuarios.setItem(i, 0, QTableWidgetItem(usuario))
            botao_remover = QPushButton("Remover")
            botao_remover.setStyleSheet("background-color: #E74C3C; color: white; ")
            botao_remover.clicked.connect(lambda checked, u=usuario: self.remover_usuario(u))
            tabela_usuarios.setCellWidget(i, 1, botao_remover)
        
        # Estilo para a tabela
        tabela_usuarios.setStyleSheet("""
            QTableWidget {
        ...
        tabela_usuarios.horizontalHeader().setStretchLastSection(True)
        tabela_usuarios.verticalHeader().setVisible(False)
        tabela_usuarios.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)

        layout.addWidget(tabela_usuarios)
