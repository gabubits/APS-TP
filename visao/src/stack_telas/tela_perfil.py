...
class TelaPerfil(QWidget):
    def __init__(self, parent,
        ...
        if self.id_usuario == 1:
            ...
            for usuario in self.controle.obter_usuarios(): 
                if self.id_usuario != usuario[0]:
                    self.lista.addItem(UsuarioItemList(usuario[0], usuario[1], usuario[2]))
    ...
    def remover_perfil(self):
        per_rem = self.lista.takeItem(
            self.lista.row(
                self.lista.selectedItems()[0]
                )
            )
        self.controle.excluir_usuario(per_rem.id)
