...
class TelaLogin(TelaBase):
    ...
    def abrir_tela_ops(self, id_usuario):
        TelaOpcoes(self, id_usuario, self.controle).show()
        self.hide()
    
    def verificacao_login(self):
        username = self.entrada_username.text()
        usuario_id = self.controle.buscar_username(username.rstrip())
        if not usuario_id:
            ...
            return
        # ******************
        if not self.controle.verificar_senha(usuario_id, self.entrada_senha.text().rstrip()):
            errorMessage = QMessageBox(self)
            errorMessage.setIcon(QMessageBox.Icon.Critical)
            errorMessage.setWindowTitle("Falha na autenticação!")
            errorMessage.setStandardButtons(QMessageBox.StandardButton.Ok)
        ...
        self.abrir_tela_ops(usuario_id)
